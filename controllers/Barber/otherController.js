...
const barberHomeController = (req, res) => {

    if (req.barberemail && req.role) {
        res.status(200).json({
            message: "Hurray Barber successfully present in HOME Dashboard",
            email: req.adminemail,
            role: req.role
        })
    } else {
        res.status(404).json({ message: "You are not authorize barber" })
    }

}

const barberSalonController = (req, res) => {

    if (req.barberemail && req.role) {
        res.status(200).json({
            message: "Barber is now present in the Salon Dashboard",
            email: req.adminemail,
            role: req.role
        })
    } else {
